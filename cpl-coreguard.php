...
final class CPL_CoreGuard {
	...
	/**
	 * Runs on plugin deactivation. Removes every file we created.
	 */
	public function deactivate(): void {
		$this->remove_mu_plugin();
		$this->remove_db_error_dropin();
		// $this->clean_wp_config();
	...
	public function write_config_file(): bool {
		...
		$icon = sanitize_url( $site_icon );

		$php  = "<?php\n";
		$php .= "/** Generated by CPL CoreGuard " . CPL_COREGUARD_VERSION . " — do not edit manually. */\n";
		$php .= "defined( 'ABSPATH' ) || exit;\n\n";
		$php .= "define( 'CPL_SITE_NAME',   " . var_export( $name,    true ) . " );\n";
		$php .= "define( 'CPL_SITE_ICON',   " . var_export( $icon,    true ) . " );\n";
		$php .= "define( 'CPL_BRAND_COLOR', " . var_export( $color,   true ) . " );\n";
		$php .= "define( 'CPL_MAINT_MSG',   " . var_export( $message, true ) . " );\n";
		$php .= "define( 'CPL_LOCALE',      " . var_export( $locale,  true ) . " );\n";
	...
	private function inject_wp_config(): void {
		...
		$snippet .= "    \$cpl = __DIR__ . '/wp-content/mu-plugins/" . CPL_COREGUARD_MU_FILE . "';\n";
		$snippet .= "    if ( is_readable( \$cpl ) ) {\n";
		$snippet .= "        require_once \$cpl;\n";
		$snippet .= "    }\n";
		$snippet .= "}\n";
	...
	public function load_textdomain(): void {
		load_plugin_textdomain(
			'cpl-coreguard',
			false,
			dirname( plugin_basename( CPL_COREGUARD_FILE ) ) . '/languages'
		);
	}
}
